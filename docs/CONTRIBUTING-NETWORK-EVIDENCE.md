...
A report that disproves a Queqiao assumption is as valuable as a performance
win. The maintainers cannot access every carrier, residential ISP, hotel,
enterprise firewall, or long-fat route, so real observations are part of the
project—not just post-release support.

Start with [Benchmarking](BENCHMARKING.md) for command forms. Use [Field
validation](FIELD-VALIDATION.md) when a result should count toward the release
...
- transport availability: UDP, TCP, ports, PMTU, NAT, and middlebox behavior;
...
## Evaluate one transport three ways

Do not configure three Queqiao architectures. Exercise the same transport and
report the workload families that apply:
...
- Compare complete stacks. BBR is a congestion controller; identify the proxy,
  framing, TLS/TCP or QUIC carrier, recovery, and BBR implementation around it.
...
Before publishing, remove invitations, profiles, private keys, certificates,
tokens, provider state, public subscriber or gateway addresses, private
hostnames, usernames, local paths, packet payloads, and raw logs containing
destinations or other users' metadata.
