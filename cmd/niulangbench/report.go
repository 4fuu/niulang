...
// 35%-burst-loss block the reference completed 7 of 12 trials and niulang 10 of
...
// gateReport fails when niulang is materially worse than the reference in any
...
func gateReport(summaries []CellSummary, tolerance float64) error {
	...
	for _, f := range flows {
		...
		subject, hasSubject := byCell[f]["niulang"]
	...
	if compared == 0 {
		return fmt.Errorf("gate requires both stacks; run with --stacks baseline,niulang")
	}
	if len(failures) > 0 {
	...
	fmt.Printf("\ngate: niulang is within %.0f%% of the reference in all %d compared cells\n", tolerance*100, compared)
	return nil
}
