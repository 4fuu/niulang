// Command niulangbench runs niulang and a TUIC-shaped reference proxy over an
...
import (
	...
	"github.com/4fuu/niulang/internal/baseline"
	"github.com/4fuu/niulang/internal/extproxy"
	"github.com/4fuu/niulang/internal/identity"
	qmetrics "github.com/4fuu/niulang/internal/metrics"
...
func main() {
	if err := run(os.Args[1:]); err != nil {
		fmt.Fprintf(os.Stderr, "niulangbench: %v\n", err)
		os.Exit(1)
	}
}
...
func run(args []string) error {
	var opts options
	fs := flag.NewFlagSet("niulangbench", flag.ContinueOnError)
	fs.StringVar(&opts.stacks, "stacks", "baseline,niulang", "comma-separated stacks to measure")
	...
	fs.Float64Var(&opts.aggregateMbits, "aggregate-rate", 0, "niulang aggregate application-data budget in Mbit/s (0 disables)")
	...
	fs.StringVar(&opts.contend, "contend", "", "two stacks to run concurrently on one shared bottleneck, e.g. niulang,baseline; reports each one's share of the link")
	fs.BoolVar(&opts.gate, "gate", false, "exit non-zero when niulang is worse than the reference beyond --tolerance")
...
func measure(stack string, opts options, pathCfg pathsim.Config, origin *origin, flows, trial int) trialResult {
	...
	// keeps a persistent connection, so charging niulang for a cold handshake
...
func startStackOn(ctx context.Context, stack string, opts options, pathCfg pathsim.Config, shared *pathsim.Bottleneck) (*harness, error) {
	...
	// by the UDP one.
	if kind := extproxy.Kind(stack); kind.Transport() == "tcp" && stack != "baseline" && stack != "niulang" {
	...
	case "baseline", "tuic":
		...
		client, err := baseline.NewClient(baseline.ClientConfig{
			ListenAddr: h.socks, RemoteAddr: relay.LocalAddr(), ServerName: "niulang.test",
	...
	case "niulang":
		identityDir, err := os.MkdirTemp("", "niulangbench-identity-")
		if err != nil {
			h.Close()
			return nil, err
		}
	...
	default:
		...
		workDir, err := os.MkdirTemp("", "niulangbench-"+stack+"-")
		if err != nil {
			h.Close()
			return nil, err
		}
...
func startTCPStack(ctx context.Context, kind extproxy.Kind, opts options, pathCfg pathsim.Config, logger *slog.Logger) (*harness, error) {
	...
	workDir, err := os.MkdirTemp("", "niulangbench-"+string(kind)+"-")
	if err != nil {
		h.Close()
		return nil, err
	}
