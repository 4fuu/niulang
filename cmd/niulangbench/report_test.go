...
// niulang 10 of 12, and reporting only successes made the transport that
...
func TestSummaryCountsFailedTrials(t *testing.T) {
	trials := []TrialRecord{
		...
		trial("niulang", 1, 7, true), trial("niulang", 1, 7, true),
		trial("niulang", 1, 3, true), trial("niulang", 1, 3, true),
	}
	...
	reference, subject := byStack["baseline"], byStack["niulang"]
	...
	if subject.MedianMbits <= reference.MedianMbits {
		t.Fatalf("all-trial median: niulang %.2f, reference %.2f; the transport that "+
...
func TestGateFailsOnGoodputShortfall(t *testing.T) {
	summaries := summarize([]TrialRecord{
		...
		trial("niulang", 1, 8, true), trial("niulang", 1, 8, true),
	})
...
func TestGateFailsOnCompletionShortfall(t *testing.T) {
	summaries := summarize([]TrialRecord{
		trial("baseline", 1, 10, true), trial("baseline", 1, 10, true),
		trial("niulang", 1, 10, true), trial("niulang", 1, 10, false),
	})
...
func TestGatePassesAtParity(t *testing.T) {
	summaries := summarize([]TrialRecord{
		...
		trial("niulang", 1, 10.5, true), trial("niulang", 4, 19.5, true),
	})
	if err := gateReport(summaries, 0.10); err != nil {
...
func TestReportRoundTripsAsJSON(t *testing.T) {
	report := Report{
		...
		Latency: []LatencyRecord{{Stack: "niulang", Trial: 1, ColdMillis: 210, WarmMillis: 201, Complete: true}},
		UDP: []UDPRecord{{
			Stack: "niulang", Trial: 1, Sent: 100, Received: 97, Lost: 3,
...
func TestSetupFailuresAreSeparatedFromTransportFailures(t *testing.T) {
	trials := []TrialRecord{
		{Stack: "niulang", Flows: 1, MbitsPerSec: 10, Complete: true},
		{Stack: "niulang", Flows: 1, MbitsPerSec: 12, Complete: true},
		{Stack: "niulang", Flows: 1, MbitsPerSec: 0, Note: "warmup: EOF"},
		{Stack: "niulang", Flows: 1, MbitsPerSec: 0, Note: "setup: dial failed"},
	}
	summaries := summarize(trials)
	got := summaries[0]
...
func TestAllSetupFailuresDoNotPanic(t *testing.T) {
	got := summarize([]TrialRecord{
		{Stack: "niulang", Flows: 1, Note: "warmup: EOF"},
		{Stack: "niulang", Flows: 1, Note: "warmup: EOF"},
	})[0]
