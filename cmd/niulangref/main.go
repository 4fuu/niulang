// Command niulangref runs the TUIC-shaped reference proxy as a standalone
// client or server so niulang can be compared against it on a real link.
//
...
// relayed TCP connection, unframed copying) on the same QUIC stack niulang
...
func main() {
	if err := run(os.Args[1:]); err != nil {
		fmt.Fprintf(os.Stderr, "niulangref: %v\n", err)
		os.Exit(1)
	}
}
...
func run(args []string) error {
	...
	fs := flag.NewFlagSet("niulangref", flag.ContinueOnError)
	...
	fs.StringVar(&serverName, "server-name", "niulang.test", "TLS server name")
