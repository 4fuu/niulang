// Command niulangpack builds the distributable niulangd archives.
//
...
func main() {
	if err := run(context.Background(), os.Args[1:]); err != nil {
		fmt.Fprintf(os.Stderr, "niulangpack: %v\n", err)
		os.Exit(1)
	}
}

func run(ctx context.Context, args []string) error {
	fs := flag.NewFlagSet("niulangpack", flag.ContinueOnError)
...
func packageRelease(ctx context.Context, cfg config) error {
	...
	tempDir, err := os.MkdirTemp(outputParent, ".niulangpack-")
	if err != nil {
...
func buildTarget(ctx context.Context, repoRoot, tempDir, resultDir string, cfg config, target target, documents []archiveFile, moduleDirs map[string]string) (map[string][sha256.Size]byte, error) {
	packageBase := fmt.Sprintf("niulangd_%s_%s_%s", cfg.version, target.goos, target.goarch)
	binaryName := "niulangd"
	...
	cmd := exec.CommandContext(ctx, "go", "build", "-trimpath", "-buildvcs=false", "-ldflags", ldflags, "-o", binaryPath, "./cmd/niulangd")
	cmd.Dir = repoRoot
...
func renderCycloneDX(cfg config, target target, binarySum [sha256.Size]byte, modules []linkedModule) ([]byte, error) {
	rootRef := fmt.Sprintf("niulangd@%s:%s", cfg.version, target)
	root := cdxComponent{
		Type: "application", BOMRef: rootRef, Group: "github.com/4fuu/niulang",
		Name: "niulangd", Version: cfg.version,
		Hashes:   []cdxHash{{Algorithm: "SHA-256", Content: fmt.Sprintf("%x", binarySum)}},
		Licenses: []cdxLicenseChoice{{License: cdxLicense{ID: "MIT"}}},
		Properties: []cdxProperty{
			{Name: "niulang:commit", Value: cfg.commit},
			{Name: "niulang:target", Value: target.String()},
			{Name: "niulang:wire-protocol", Value: fmt.Sprint(protocol.Version)},
			{Name: "niulang:go-version", Value: runtime.Version()},
		},
	}
	...
	serialInput := append([]byte("niulang/cyclonedx/v1\x00"+cfg.version+"\x00"+cfg.commit+"\x00"+target.String()+"\x00"), binarySum[:]...)
...
func renderThirdPartyLicenses(modules []linkedModule) ([]byte, error) {
	...
	output.WriteString("NIULANG THIRD-PARTY LICENSES\n\n")
