...
func TestEnrollmentURIAllowsFollowingFlags(t *testing.T) {
	err := runEnroll([]string{"niulang://invalid", "--profile", "profile.json"})
...
func TestClientMissingConfigurationExplainsEnrollment(t *testing.T) {
	t.Setenv("NIULANG_LOG_DIR", t.TempDir())
	err := runClient(nil)
	if err == nil || !strings.Contains(err.Error(), "--profile") || !strings.Contains(err.Error(), "--providers") || !strings.Contains(err.Error(), "niulangd enroll") {
...
func TestPerformanceSnapshotIsMachineReadable(t *testing.T) {
	...
	for key, want := range map[string]any{
		...
		"sample_interval_seconds": float64(5), "niulang_active_flows": float64(1),
