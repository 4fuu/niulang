...
func testServiceConfig() serviceConfig {
	return serviceConfig{
		...
		binary:        "/home/alice/.niulang/bin/niulangd",
		providers:     "/home/alice/.config/niulang/providers.json",
...
func TestServiceArgumentsUseProfileOrManifest(t *testing.T) {
	...
	if !strings.Contains(manifest, "--providers /home/alice/.config/niulang/providers.json") {
	...
	config.providers = ""
	config.profile = "/home/alice/.config/niulang/hk.json"
	single := strings.Join(config.arguments(), " ")
	if !strings.Contains(single, "--profile /home/alice/.config/niulang/hk.json") {
...
// The client's default listener and the port in deploy/clash-niulang.yaml have
...
func TestRenderLaunchAgentQuotesEveryArgument(t *testing.T) {
	config := testServiceConfig()
	config.binary = "/Users/alice/Library/Application Support/q & r/niulangd"
