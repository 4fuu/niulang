...
// from "what does the Go QUIC implementation achieve here", so a niulang/TUIC
// gap can be attributed to niulang's own framing, scheduling, and windows
...
const (
	// ALPN is deliberately distinct from the niulang ALPN so a misconfigured
	// benchmark cannot silently measure the wrong listener.
	ALPN = "niulang-baseline/1"
...
const (
	...
	// otherwise a niulang-with-Brutal versus reference-with-BBR result measures
...
func (t Transport) quicConfig() *quic.Config {
	...
	return &quic.Config{
		...
		// like TUIC. niulang does ramp, which is a real advantage over TUIC and
...
type ClientConfig struct {
	...
	// under test. niulang has the same option, and a comparison is only valid
