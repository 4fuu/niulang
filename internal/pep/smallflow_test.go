...
func TestSmallExchangesAreRepairedOnceThePathIsKnown(t *testing.T) {
	...
	pathmodel.Shared(key).Report(99, pathmodel.Observation{Erasure: 0.42, Floor: 0.42, BurstFactor: 1, ObservedSamples: 5000, Delivered: 0, RoundTrip: 0})
...
func TestAShortFlowCostsARoundTrip(t *testing.T) {
	...
	loopback := &net.UDPAddr{IP: net.IPv4(127, 0, 0, 1), Port: 0}
	pathmodel.Shared(pathKey(loopback, loopback)).Report(99, pathmodel.Observation{Erasure: path.LossRate, Floor: path.LossRate, BurstFactor: 1, ObservedSamples: 5000, Delivered: 0, RoundTrip: 0})
