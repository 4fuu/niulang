...
func TestALPNFailureExplainsEndpointOrVersionMismatch(t *testing.T) {
	if quicDataALPN != "h3" {
		t.Fatalf("data ALPN = %q, want real HTTP/3", quicDataALPN)
	}
	err := explainDataHandshakeError("gateway.example:443", "QUIC", errors.New("remote error: tls: no application protocol"))
	message := err.Error()
	if !strings.Contains(message, "protocol 3") || !strings.Contains(message, "gateway.example:443") || !strings.Contains(message, "incompatible") {
		t.Fatalf("unhelpful ALPN error: %v", err)
	}
	original := errors.New("connection refused")
	if got := explainDataHandshakeError("gateway.example:443", "QUIC", original); got != original {
