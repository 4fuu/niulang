...
// TestCodedDatagramVectors replays the committed protocol-3 datagram vectors
...
func TestCodedDatagramVectors(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join("..", "..", "testdata", "protocol3", "vectors.json"))
	if err != nil {
	...
	for _, v := range file.CodedDatagrams {
		t.Run(v.Name, func(t *testing.T) {
			...
			if v.Reject {
				if len(frames) != 0 {
					t.Fatalf("a datagram protocol 3 forbids delivered %d frames (%s)", len(frames), v.Why)
				}
				return
			}
			if after != before {
