...
OUTPUT_DIR="$MOBILE_DIR/ios/Frameworks"
...
BUILD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/queqiao-ios-core.XXXXXX")
...
PATH="$TOOLS_DIR:$PATH" "$TOOLS_DIR/gomobile" bind \
    ...
    -target=ios \
    -iosversion=17.0 \
    -o "$BUILD_DIR/Mobilecore.xcframework" \
    .

"$SCRIPT_DIR/audit-mobile-binary.sh" "$BUILD_DIR/Mobilecore.xcframework"

if [ -e "$OUTPUT_DIR/Mobilecore.xcframework" ]; then
    mv "$OUTPUT_DIR/Mobilecore.xcframework" "$BUILD_DIR/previous.xcframework"
fi
mv "$BUILD_DIR/Mobilecore.xcframework" "$OUTPUT_DIR/Mobilecore.xcframework"
echo "Wrote $OUTPUT_DIR/Mobilecore.xcframework"
